...
import (
	...
	_ "github.com/inercia/go-llm/pkg/providers/openai" // Register OpenAI provider
)
...
func main() {
	// Create client using the factory pattern - works with any provider
	// Just change the Provider field to use different providers (e.g., "gemini", "ollama", etc.)
	factory := llm.NewFactory()
	client, err := factory.CreateClient(llm.ClientConfig{
		Provider: "openai", // Can be "openai", "gemini", "ollama", "openrouter", "deepseek", or "mock"
	...
	err = demonstrateTextAnalysis(ctx, client)
	if err != nil {
	...
	fmt.Println("\n=== Example 2: Math Problem Solving ===")
	err = demonstrateMathSolving(ctx, client)
	if err != nil {
	...
	fmt.Println("\n=== Example 3: Product Recommendations ===")
	err = demonstrateProductRecommendations(ctx, client)
	if err != nil {
	...
	err = demonstrateBasicJSONMode(ctx, client)
	if err != nil {
		log.Printf("Basic JSON mode example failed: %v", err)
	}
}

func demonstrateTextAnalysis(ctx context.Context, client llm.Client) error {
...
func demonstrateMathSolving(ctx context.Context, client llm.Client) error {
...
func demonstrateProductRecommendations(ctx context.Context, client llm.Client) error {
...
func demonstrateBasicJSONMode(ctx context.Context, client llm.Client) error {
