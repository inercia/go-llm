...
var (
	...
	// File support patterns - models that support file inputs
	fileSupport = []ModelAttribute[bool]{
		{regexp.MustCompile(`^gpt-.*`), true}, // All GPT models support files through context
		{regexp.MustCompile(`.*`), true},      // Default: support files
	}

	// Streaming support patterns - models that support streaming
	streamingSupport = []ModelAttribute[bool]{
		{regexp.MustCompile(`^gpt-.*`), true}, // All GPT models support streaming
		{regexp.MustCompile(`.*`), true},      // Default: support streaming
	}
)
...
func init() {
	...
	llm.RegisterProvider("openai", func(config llm.ClientConfig) (llm.Client, error) {
