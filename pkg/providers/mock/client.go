...
func init() {
	...
	llm.RegisterProvider("mock", func(config llm.ClientConfig) (llm.Client, error) {
		return NewClient(config.Model, "mock")
	})
	llm.RegisterProvider("mocked", func(config llm.ClientConfig) (llm.Client, error) {
		return NewClient(config.Model, "mock")
	})
}
