...
func TestMockStreamChatCompletion(t *testing.T) {
	...
	if err != nil {
		t.Fatal("Failed to create mock client:", err)
	}
	...
	req := ChatRequest{
		Model: "test-model",
		Messages: []Message{
			{Role: RoleUser, Content: []MessageContent{NewTextContent("test stream")}},
		},
		Stream: true,
	}
		...
		if event.IsDelta() {
			...
			if len(event.Choice.Delta.Content) > 0 {
				if textContent, ok := event.Choice.Delta.Content[0].(*TextContent); ok {
					t.Log("Delta content:", textContent.GetText())
				}
			}
	...
	if deltaCount == 0 {
		t.Error("No delta events received")
	}
}
