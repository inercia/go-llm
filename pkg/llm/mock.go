...
// NewMockClient creates a new mock LLM client for testing
func NewMockClient(modelName, provider string) *MockClient {
	return &MockClient{
		modelInfo: ModelInfo{
			Name:              modelName,
			Provider:          provider,
...
func (m *MockClient) handleToolResponse(req ChatRequest) (*ChatResponse, error) {
	// Find the last tool message
	var toolResult string
	for i := len(req.Messages) - 1; i >= 0; i-- {
		if req.Messages[i].Role == RoleTool {
			if len(req.Messages[i].Content) > 0 {
				if textContent, ok := req.Messages[i].Content[0].(*TextContent); ok {
...
func (m *MockClient) generateIntelligentResponse(req ChatRequest) (*ChatResponse, error) {
	var lastUserMessage string
	for i := len(req.Messages) - 1; i >= 0; i-- {
		if req.Messages[i].Role == RoleUser && len(req.Messages[i].Content) > 0 {
			if textContent, ok := req.Messages[i].Content[0].(*TextContent); ok {
...
func (m *MockClient) generateToolCallResponse(req ChatRequest, trigger, userMessage string) (*ChatResponse, error) {
		...
		Choices: []Choice{
			{
				...
				Message: Message{
					Role:      RoleAssistant,
					Content:   []MessageContent{NewTextContent("I need to use a tool to help with your request.")},
...
func (m *MockClient) StreamChatCompletion(ctx context.Context, req ChatRequest) (<-chan StreamEvent, error) {
...
func (m *MockClient) sendStreamEvents(ctx context.Context, events []StreamEvent) <-chan StreamEvent {
...
func (m *MockClient) generateStreamingResponse(ctx context.Context, req ChatRequest) <-chan StreamEvent {
	...
	go func() {
		...
		for _, word := range words {
			...
			case <-ctx.Done():
				return
			case ch <- NewDeltaEvent(0, &MessageDelta{
				Content: []MessageContent{NewTextContent(word + " ")},
			}):
			}
		...
		if shouldCallTool {
			...
			case <-ctx.Done():
				return
			case ch <- NewDoneEvent(0, "tool_calls"):
			}
		} else {
			select {
			case <-ctx.Done():
				return
			case ch <- NewDoneEvent(0, "stop"):
			}
		}
	}()
...
func (m *MockClient) StreamChatCompletionWithTools(ctx context.Context, req ChatRequest, toolStream <-chan StreamEvent) (<-chan StreamEvent, error) {
	// Log the request
	m.callLog = append(m.callLog, req)
	...
	var toolStreams []<-chan StreamEvent
	if toolStream != nil {
...
func (m *MockClient) Reset() *MockClient {
	m.responses = []ChatResponse{}
	m.responseIndex = 0
	m.errors = []error{}
	m.errorIndex = 0
	m.callLog = []ChatRequest{}
	return m
}
...
func (m *MockClient) WithSimpleResponse(content string) *MockClient {
	return m.AddResponse(ChatResponse{
		...
		Choices: []Choice{
			{
				...
				Message: Message{
					Role:    RoleAssistant,
					Content: []MessageContent{NewTextContent(content)},
				},
				FinishReason: "stop",
			},
		},
...
func (m *MockClient) WithToolCall(toolName string, args map[string]interface{}) *MockClient {
	argsJSON := "{}"
	if len(args) > 0 {
		...
		Choices: []Choice{
			{
				...
				Message: Message{
					Role:    RoleAssistant,
					Content: []MessageContent{NewTextContent("I need to use a tool to help with this request.")},
					ToolCalls: []ToolCall{
						{
...
func (m *MockClient) WithError(code, message, errorType string) *MockClient {
	return m.AddError(&Error{
		Code:    code,
		Message: message,
		Type:    errorType,
	})
...
func (m *MockClient) WithLatency(duration time.Duration) *MockClient {
	m.latencySimulation = duration
	return m
}

// WithFailureRate configures random failure simulation (0.0 to 1.0)
func (m *MockClient) WithFailureRate(rate float64) *MockClient {
	m.failureRate = rate
	return m
}

// WithModelCapabilities configures the model's capabilities
func (m *MockClient) WithModelCapabilities(maxTokens int, supportsTools, supportsVision, supportsFiles, supportsStreaming bool) *MockClient {
...
func (m *MockClient) WithConversationState(key string, value interface{}) *MockClient {
	m.conversationState[key] = value
	return m
}

// WithStreamResponse adds a pre-configured streaming response
func (m *MockClient) WithStreamResponse(events []StreamEvent) *MockClient {
	m.streamResponses = append(m.streamResponses, events)
	return m
}

// WithToolCallHandler registers a handler for specific tool calls
func (m *MockClient) WithToolCallHandler(toolName string, handler func(args string) (string, error)) *MockClient {
...
func (m *MockClient) WithConversation(exchanges []ConversationExchange) *MockClient {
...
func (m *MockClient) WithSystemMessage(content string) *MockClient {
	return m.AddResponse(ChatResponse{
...
func (m *MockClient) WithFunctionResult(functionName, result string) *MockClient {
...
func (m *MockClient) WithMultiStepResponse(steps []string) *MockClient {
...
func CreateWordByWordStream(text string, delay time.Duration) []StreamEvent {
	words := strings.Split(text, " ")
	events := make([]StreamEvent, 0, len(words)+1)

	for _, word := range words {
		events = append(events, NewDeltaEvent(0, &MessageDelta{
			Content: []MessageContent{NewTextContent(word + " ")},
		}))
	}

	events = append(events, NewDoneEvent(0, "stop"))
	return events
}

// CreateToolCallStream creates a streaming response that includes a tool call
func CreateToolCallStream(initialText, toolName string, args map[string]interface{}) []StreamEvent {
	...
	events = append(events, NewDoneEvent(0, "tool_calls"))
	return events
}
...
func (m *MockClient) AssertCallCount(expected int) bool {
	return len(m.callLog) == expected
}

// AssertLastMessageContains checks if the last user message contains specific text
func (m *MockClient) AssertLastMessageContains(text string) bool {
...
func (m *MockClient) AssertToolWasCalled(toolName string) bool {
